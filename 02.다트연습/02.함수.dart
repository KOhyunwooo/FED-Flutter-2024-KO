...
void main() {
  ...
  showTxt('이순신 시리즈 영화 주요출연 배우들:+${actorList(actors.toList())}');

  // 다음 추가 이순신 시리즈가 있다면 어느배우를  추천하는가?
  showTxt('다음 이순신 시리즈가 있다면 추천배우는 공유다!');
  showTxt(
      '공유의 취미는 ${recommActor['공유']?['취미']}다, 사는곳은 ${recommActor['공유']?['사는곳']}이고, 공유의 자동차는 ${recommActor['공유']?['자동차'] ?? "정보없어요~"}이다!');
  showTxt('다음 이순신 시리즈가 있다면 추천배우는 조인성이다!');
  showTxt(
      '조인성의 취미는 ${recommActor['조인성']?['취미']}다, 사는곳은 ${recommActor['조인성']?['사는곳']}이고, 조인성의 자동차는 ${recommActor['조인성']?['자동차'] ?? "정보없음"}이다!');
  // [다트문법 널세이프티 적용사례!!!]
  // -> Map형 데이터일 경우 내부 속성 불일치 등으로
  // null이 나올때 에러로 연결되므로 이런경우에
  // 다트는 필수적으로 널 세이프티 처리를 강요한다!
  // 물음표(?)하나는 null일 경우 "null" 문자열 출력대체
  // 물음표두개(??)는 null 발생시 문자열 대체한 경우
...
const recommActor = {
  ...
  '박해일': {'나이': 39, '취미': '비오게하기', '사는곳': '비오는방'},
  '변요한': {'나이': 40, '취미': '날기', '사는곳': '비오는방'},
  '최민식': {'나이': 41, '취미': '날기', '사는곳': '한국'},
};
// [다트의 상수: const, final]
...
String makeSubTit(int seq) {
  ...
  return "영화 ${leeFight[seq]}의 부제목은?" +
      '${subTit[leeFight[seq]] ?? "부제목 없어요~!"}'; // ?? subTit[leeFight[seq]] 가 null이니? 라는뜻
